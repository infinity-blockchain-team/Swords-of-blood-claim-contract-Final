...
contract ClaimContract is Ownable {
    ...
    IERC20 public token= IERC20(0xEa50715D2af763c93bBE391cB405C389C3E7F447);
...
function uploadBatch(
    ...
    require(
        ...
        users.length == tgePercents.length &&
    ...
    for (uint256 i = 0; i < users.length; i++) {
        ...
        uint256 tge = tgePercents[i];
        uint256 months = vestingMonths[i];

        require(user != address(0), "Invalid address");
        require(allocation > 0, "Allocation must be greater than zero");
        require(tge >= 1 && tge <= 100, "TGE % must be between 1 and 100");

        if (tge <= 89) {
            require(
                months == 3 || months == 6 || months == 9 || months == 12 || months == 18 || months == 24,
                "Invalid months for TGE <= 89%"
            );
        } else if (tge <= 99) {
            require(months == 3 || months == 6 || months == 9, "Only 3 ,6 or 9 months allowed for TGE 90 to 99%");
        } else if (tge == 100) {
            require(months == 0, "No vesting months for TGE 100%");
        }

        if (vestingList[user].totalAllocation == 0) {
                vestingUsers.push(user);
            }

        vestingList[user] = VestingInfo({
    ...
    function claimTokens() external {
        ...
        require(info.totalAllocation > 0, "You are not whitelisted");

        uint256 claimable = getClaimableAmount(msg.sender);
        require(claimable > 0, "No tokens available to claim");

        info.claimed = info.claimed.add(claimable);
...
function getClaimableAmount(address user) public view returns (uint256) {
    ...
    if (!vestingStarted || info.totalAllocation == 0) return 0;

    // If TGE is 100%, everything is unlocked instantly
    if (info.tgePercent == 100) {
        return info.totalAllocation.sub(info.claimed);
    }

    uint256 start = vestingStartTime;
    uint256 timePassed = block.timestamp.sub(start);
    uint256 monthsPassed = timePassed.div(30 days);
   
    uint256 unlockedPercent = info.tgePercent;

    if (monthsPassed > 0 && info.vestingMonths > 0) {
        uint256 linearPercent = monthsPassed.mul(uint256(100).sub(info.tgePercent)).div(info.vestingMonths);
        if (info.tgePercent.add(linearPercent) > 100) linearPercent = 100 - info.tgePercent;
        unlockedPercent = info.tgePercent.add(linearPercent);
    }

    uint256 totalUnlocked = info.totalAllocation.mul(unlockedPercent).div(100);
    uint256 claimableNow = totalUnlocked.sub(info.claimed);

    return claimableNow;
}


 function viewUserInfo(address user) public view returns (
    uint256 allocation,
    uint256 claimed,
    uint256 remaining,
    uint256 tgePercent,
    ...
    return (
        ...
        info.claimed,
        info.totalAllocation.sub(info.claimed),
        info.tgePercent,
        info.vestingMonths,
        claimable
    );
  ...
  function withdrawUnallocated(uint256 amount) external onlyOwner {
    ...
    function getTotalVestingUsers() public view returns (uint256) {
        return vestingUsers.length;
    }
